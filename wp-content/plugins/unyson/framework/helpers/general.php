 ...
 * @return string HTML
 */
function slz_render_view($file_path, $view_variables = array(), $return = true, $img_lazyload = true, $cache = true) {
    $cache_name = md5($file_path . serialize($view_variables));
    $optimizer = slz_ext('optimization');
    $result = $optimizer->getCache($cache_name);
    if (empty($result) || !$cache || WP_DEBUG || is_admin()) {
        try {
            $result = SLZ_Cache::get($cache_name);
            ...
            require $file_path;
            $result = ob_get_clean();
            if ($img_lazyload && ! is_admin() ) {
                $result = preg_replace('/<img(.*?) src="(.*?)"/i', '<img$1 data-lazyload data-src="$2"', $result);
            }
            SLZ_Cache::set($cache_name, $result);
            if (function_exists('lema_minify_html')) {
                $result = lema_minify_html($result);
            }
...
function slz_get_options_values_from_input(array $options, $input_array = null) {
	...
	$db_default = slz_get_db_settings_option();
    $input_array = slz_array_merge_recursive_distinct($db_default, $input_array);
    $values = array();
...
function slz_array_merge_recursive_distinct ( array &$array1, array &$array2)
{
    $merged = $array1;
    foreach ($merged as $key => $val) {
        if ( is_array($val) &&!isset($array2[$key]) && !slz_check_not_array_index($val)) {
            $merged[$key] = array();
        }
    }

    foreach ($array2 as $key => &$value) {
        if (is_array($value) && isset ($merged [$key]) && is_array($merged [$key]) && slz_check_not_array_index($value)) {
            $merged [$key] = slz_array_merge_recursive_distinct($merged [$key], $value);
        } else {
            if (is_string($value)) {
...
function slz_check_not_array_index($arr) {
    foreach ($arr as $k => $v) {
        if (is_int($k)) {
            return false;
        }
