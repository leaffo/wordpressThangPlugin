<?php
$out = $icon_class = $has_background = $custom_css = $css = $under_line = '';
...
//-------custom general css------------//
	...
	if( !empty( $data['icon_bg_cl_2'] ) ){
		$css = '
				.%1$s .icon-background .wrapper-icon {
					background-color: %2$s !important;
				}
			';
		$custom_css .= sprintf( $css, esc_attr( $data['uniq_id'] ), esc_attr( $data['icon_bg_cl_2']) );
	}

	// icon background hover color
	if( !empty( $data['icon_bg_hv_cl_2'] ) ){
		$css = '
				.%1$s .icon-background .wrapper-icon:hover{
					background-color: %2$s !important;
				}
			';
		$custom_css .= sprintf( $css, esc_attr( $data['uniq_id'] ), esc_attr( $data['icon_bg_hv_cl_2']) );
	}

	// icon background color
	if( !empty( $data['icon_bd_cl_2'] ) ){
		$css = '
				.%1$s .icon-background .wrapper-icon {
					border-color: %2$s !important;
				}
			';
		$custom_css .= sprintf( $css, esc_attr( $data['uniq_id'] ), esc_attr( $data['icon_bd_cl_2']) );
	}

	// icon background hover color
	if( !empty( $data['icon_bd_hv_cl_2'] ) ){
		$css = '
				.%1$s .icon-background .wrapper-icon:hover{
					border-color: %2$s !important;
				}
			';
		$custom_css .= sprintf( $css, esc_attr( $data['uniq_id'] ), esc_attr( $data['icon_bd_hv_cl_2']) );
	}
