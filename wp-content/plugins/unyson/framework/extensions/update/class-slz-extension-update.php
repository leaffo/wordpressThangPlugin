...
class SLZ_Extension_Update extends SLZ_Extension
{
	...
	private function add_actions()
	{
		...
		//  Update products notices
		add_action('admin_notices', array($this, '_action_slz_update_products'));
	}
	...
	public function _action_slz_update_products(){
		$check = slz()->theme->get_config('check_update_data');
		if ( is_plugin_active( 'woocommerce/woocommerce.php' ) && $check && !get_option( 'slz_update_products_db', false ) && version_compare(WC_VERSION, '3.0.0') >= 0 )
		{
			echo '<div id="message" class="notice notice-warning is-dismissible">';
			echo '<p><strong>'.esc_html__( 'Products update', 'slz' ).'</strong> &#8211;'.esc_html__( 'We need to update your store\'s database to the latest version.', 'slz' ).'</p>';
			echo '<p><button class="slz-upd-products-db button-primary">'. esc_html__( 'Run the updater', 'slz' ).'</button></p>';
			echo '</div>';
		}
	}
	public function ajax_update_products(){
		...
		$products = SLZ_Com::get_post_id2title( $args, array(), false );
		...
		update_option( 'slz_update_products_db', true );
		echo 'success';
		exit();
	}
}
