...
class SLZ_Header_Header_07 extends SLZ_Header {

	public function render( $echo = true )
	{

		$this->enqueue_static();
		$options = slz_get_db_settings_option('slz-header-style-group/header_07', '');

		$palette_color = SLZ_Com::get_palette_color();

		if ( !empty ( $options ) ) {

			$topbar = isset($options['enable-header-top-bar']) ? $options['enable-header-top-bar'] : array();
			...
			if( ! $echo ) {
				return slz_render_view( $this->locate_path('/views/view.php'), compact( 'topbar', 'options', 'search_style' ) );
			}
			echo slz_render_view( $this->locate_path('/views/view.php'), compact( 'topbar', 'options', 'search_style' ) );
		}
